...
async function startServer() {
  ...
  app.get("/api/artist", async (req, res) => {
    ...
    try {
      ...
      // Prioritize official content by adding keywords
      const searchResults = await ytSearch(`${artistName} official audio`);
  ...
  // API route to get artist channel info (logo)
  app.get("/api/artist/info", async (req, res) => {
    const artistName = req.query.name as string;
    if (!artistName) return res.status(400).json({ error: "Artist name is required" });
    try {
      const searchResults = await ytSearch(artistName);
      const channel = searchResults.channels?.[0];
      res.json({ 
        name: channel?.name || artistName,
        thumbnail: channel?.thumbnail || `https://ui-avatars.com/api/?name=${encodeURIComponent(artistName)}&background=random`
      });
    } catch (err) {
      res.json({ name: artistName, thumbnail: `https://ui-avatars.com/api/?name=${encodeURIComponent(artistName)}&background=random` });
    }
  });
